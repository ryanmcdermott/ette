...
inline void inv_shift_rows(State& state) {
    ...
    for (int r = 1; r < kWordSize; ++r) {
        ...
        for (int c = 0; c < kBlockSize; ++c) {
            state[c] =
                (state[c] & m1) | (ori[(c + kBlockSize - r) % kWordSize] & m2);
        }
    }
}
...
inline void inv_mix_columns(State& state) {
    for (int i = 0; i < kBlockSize; ++i) {
        ...
        state[i] =
            (mul(v0, 0x0E) ^ mul(v1, 0x0B) ^ mul(v2, 0x0D) ^ mul(v3, 0x09))
                << 0 |
            (mul(v0, 0x09) ^ mul(v1, 0x0E) ^ mul(v2, 0x0B) ^ mul(v3, 0x0D))
                << 8 |
            (mul(v0, 0x0D) ^ mul(v1, 0x09) ^ mul(v2, 0x0E) ^ mul(v3, 0x0B))
                << 16 |
            (mul(v0, 0x0B) ^ mul(v1, 0x0D) ^ mul(v2, 0x09) ^ mul(v3, 0x0E))
                << 24;
    }
}
...
inline RoundKeys expand_key(const unsigned char* key, const int key_size) {
    ...
    const Word rcon[] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10,
...
inline void xor_data(unsigned char data[kStateSize],
...
class Block {
   public:
    ...
    Block(const std::bitset<128>& bits) {
        ...
        for (std::size_t i = 0; i < 16; ++i) {
            v_[15 - i] = static_cast<unsigned char>(
                ((bits >> (i * 8)) & mask).to_ulong());
        }
    }

    inline unsigned char* data() { return v_; }

    inline const unsigned char* data() const { return v_; }
   ...
   private:
    ...
    inline void init_v(const unsigned char* bytes,
                       const std::size_t bytes_size) {
        memset(v_, 0, sizeof(v_));

        const std::size_t cs =
            (std::min)(bytes_size, static_cast<std::size_t>(16));
...
std::bitset<N1 + N2> operator||(const std::bitset<N1>& v1,
...
template <std::size_t N>
std::bitset<N> make_bitset(const unsigned char* bytes,
                           const std::size_t bytes_size) {
    std::bitset<N> bits;
...
/** Algorithm 3 @private */
inline std::vector<unsigned char> gctr(const detail::RoundKeys& rkeys,
                                       const Block& ICB, const unsigned char* X,
    ...
    if (!X || X_size == 0) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            if (i < n - 1) {
                ...
                op_size = (X_size % kBlockByteSize) ? (X_size % kBlockByteSize)
                                                    : kBlockByteSize;
            }
...
inline void push_back(std::vector<unsigned char>& bytes,
                      const unsigned char* data, const std::size_t data_size) {
...
inline Block calc_J0(const Block& H, const unsigned char* iv,
...
inline void calc_gcm_tag(const unsigned char* data, const std::size_t data_size,
                         ...
                         unsigned char* tag, const std::size_t tag_size) {
    using namespace detail;
    using detail::gcm::operator||;

    const detail::RoundKeys rkeys =
        detail::expand_key(key, static_cast<int>(key_size));
    ...
    const std::vector<unsigned char> T =
        gcm::gctr(rkeys, J0, S.data(), gcm::kBlockByteSize);
...
inline Error check_encrypt_cond(const unsigned long data_size,
    ...
    if (pads) {
        const unsigned long required_size =
            get_padded_encrypted_size(data_size);
...
inline bool check_padding(const unsigned long padding,
...
inline Error check_gcm_cond(const std::size_t key_size,
                            const std::size_t iv_size,
    ...
    // check tag size
    if ((tag_size < 12 || 16 < tag_size) && (tag_size != 8) &&
        (tag_size != 4)) {
...
inline Error encrypt_ecb(const unsigned char* data,
    ...
    const Error e =
        detail::check_encrypt_cond(data_size, key_size, encrypted_size, pads);
    if (e != kErrorOk) {
        return e;
    }
    ...
    if (pads) {
        ...
        std::vector<unsigned char> ib(detail::kStateSize, pad_v),
            ob(detail::kStateSize);
...
inline Error decrypt_ecb(const unsigned char* data,
    ...
    const Error e = detail::check_decrypt_cond(data_size, key_size,
                                               decrypted_size, padded_size);
    if (e != kErrorOk) {
        return e;
    }
...
inline Error decrypt_cbc(const unsigned char* data,
    ...
    for (unsigned long i = 1; i < bc; ++i) {
        ...
        detail::xor_data(decrypted + offset,
                         data + offset - detail::kStateSize);
    }
    ...
    if (data_size > detail::kStateSize) {
        detail::decrypt_state(rkeys, data + (bc * detail::kStateSize), last);
        detail::xor_data(last,
                         data + (bc * detail::kStateSize - detail::kStateSize));
    } else {
...
inline Error encrypt_gcm(unsigned char* data, const std::size_t data_size,
    ...
    detail::gcm::calc_gcm_tag(data, data_size, aadata, aadata_size, key,
                              key_size, iv, iv_size, tag, tag_size);
...
inline Error encrypt_gcm(unsigned char* data, const std::size_t data_size,
    ...
    return encrypt_gcm(data, data_size, aadata, aadata_size, key, key_size, *iv,
                       12, *tag, 16);
}
...
inline Error decrypt_gcm(unsigned char* data, const std::size_t data_size,
    ...
    detail::gcm::calc_gcm_tag(C, C_size, aadata, aadata_size, key, key_size, iv,
                              iv_size, tagd, 16);
...
inline Error decrypt_gcm(unsigned char* data, const std::size_t data_size,
    ...
    return decrypt_gcm(data, data_size, aadata, aadata_size, key, key_size, *iv,
                       12, *tag, 16);
}
 ...
 * @since 1.0.0
 */
inline Error crypt_ctr(unsigned char* data, const std::size_t data_size,
                       const unsigned char* key, const std::size_t key_size,
